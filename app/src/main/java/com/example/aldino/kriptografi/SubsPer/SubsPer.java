...
//        --------------------- Permutasi ---------------------------
        ...
        for (int i = 0; i < chipertext.length(); i++) {
            if (check == 0){
                rail [row][i] = chipertext.charAt(i);
                row ++;
                if (row == shift){
                    check = 1;
                    row--;
        ...
        System.out.println("Permutasi : " + teksEnkripsi);
...
//            System.out.println("TEST");
        ...
        for (int j = 0; j < rail.length; j++) {
            for (int k = 0; k < plaintext.length() / 2; k++) {
                if (pointer < plaintext.length()) {
                    rail[j][k] = plaintext.charAt(pointer++);
                }
        ...
        System.out.println("Teks Dekripsi permutasi = " + teksDekripsi);
...
//        ----------------- Substitusi ------------------
        ...
        String chipertext = "";
        int length = teksDekripsi.length();
        for (int i = 0; i < length; i++) {
            char ch = teksDekripsi.charAt(i);
            if (Character.isLetter(ch)) {
                if (Character.isLowerCase(ch)) {
                    char c = (char) (ch - shift);
                    if (c < 'a') {
